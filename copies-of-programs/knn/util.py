...
def normalizedValue(xs, test):
    minval = min(xs)
    maxval = max(xs)
    minmax = (maxval-minval) * 1.0
    return (test - minval)/minmax
...
def normalizeTable(table, exceptFor=None):
    ...
    for row in table:
        ...
        for index, el in enumerate(row):
            ...
            # Don't normalize the exception
            if exceptFor == index:
                newRow.append(el)
                continue

            # Don't normalize string rows
            if isinstance(el, basestring):
                newRow.append(el)
                continue

            # Normalize col rows
            col = getCol(table, index)
            newRow.append(normalizedValue(col, el))
